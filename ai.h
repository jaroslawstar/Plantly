...
#include "api_key.h"
#else
#include <iostream> // Fallback headers
#include <string>
#endif


// Add any Python-related utility functions or declarations here
namespace ai {
#ifdef PYTHON_INSTALLED

    inline std::string callPythonFunctionHello() {
        if (!Py_IsInitialized()) {
            Py_Initialize();
        }
    ...
    inline int callAddNumbers(int a, int b) {
        if (!Py_IsInitialized()) {
            Py_Initialize();
    ...
    inline std::string callGeneratePlantInfo(const std::string& type) {
        if (!Py_IsInitialized()) {
            Py_Initialize();
        }
    ...
    inline std::string callGenerateResponseToQuestion(const std::string& type, const std::string& message) {
        if (!Py_IsInitialized()) {
            Py_Initialize();
        }
    ...
    inline void ExecutePy_FinalizeEx() {
        Py_FinalizeEx();
    }
#else
    inline void initializePython() {
        std::cerr << "Python is not installed. Cannot initialize Python." << std::endl;
    }

    inline void finalizePython() {
        std::cerr << "Python is not installed. Cannot finalize Python." << std::endl;
    }
    inline std::string callPythonFunctionHello() {
        return "No Python 3.9.13 installed and added to path";
    }
    ...
    inline std::string callGeneratePlantInfo(const std::string& type) {
        return "Can not generate Plant Info. Python 3.9.13 is not installed and added to the PATH";
    }

    inline std::string callGenerateResponseToQuestion(const std::string& type, const std::string& message) {
        return "Can not generate answer. Python 3.9.13 is not installed and added to the PATH";
    }
